    ...
    <div className="h-screen flex bg-bg overflow-hidden">
      <aside className="w-60 shrink-0 bg-surface border-r border-border flex flex-col">
        ...
        <nav className="p-3 space-y-0.5">
          ...
          <AdminLink href="/admin/workflows" icon={Workflow} label="Workflows" />
        </nav>
        <div className="mt-auto p-3">
          <Link href="/dashboard" className="flex items-center gap-2 text-sm text-muted hover:text-text px-3 py-2 rounded-lg hover:bg-card transition">
...
function AdminLink({ href, icon: Icon, label }: { href: string; icon: any; label: string }) {
  return (
    <Link href={href} className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-muted hover:bg-card hover:text-text transition">
