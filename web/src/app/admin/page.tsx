...
export default function AdminOverview() {
  ...
  const cards = [
    { label: "Users", icon: Users, key: "users", color: "#7c5cff" },
    { label: "Workflows", icon: Workflow, key: "workflows", color: "#ff6ec4" },
    { label: "Active workflows", icon: Zap, key: "activeFlows", color: "#22c55e" },
        ...
        {cards.map((c) => (
          <div key={c.key} className="glass rounded-2xl p-5">
            <div className="flex items-center gap-3 text-muted text-sm">
              <div className="size-9 rounded-lg grid place-items-center" style={{ background: c.color + "22" }}>
                <c.icon className="size-4" style={{ color: c.color }} />
              </div>
              {c.label}
            </div>
            <div className="mt-3 text-3xl font-bold">{stats?.[c.key] ?? "—"}</div>
          </div>
        ))}
      </div>
    </div>
