...
import { Sparkles, Plus, ShieldCheck, Workflow as WorkflowIcon, LogOut, History } from "lucide-react";
      ...
      <aside className="w-64 shrink-0 bg-surface border-r border-border flex flex-col">
        ...
        <div className="p-3">
          <button
            ...
            className="w-full bg-grad-primary text-white rounded-xl py-2.5 px-3 text-sm font-medium flex items-center justify-center gap-2 shadow-glow hover:opacity-95 transition"
          >
        ...
        <nav className="px-3 pb-3 flex-1 overflow-auto">
          <NavItem href="/dashboard" active={pathname === "/dashboard"} icon={Sparkles} label="Builder" />
          ...
          <div className="mt-6 px-2 text-xs uppercase tracking-wide text-muted/70 flex items-center gap-2">
          ...
          <div className="mt-2 space-y-0.5">
            {chats.slice(0, 12).map((c) => (
              <Link
                ...
                className={`block text-sm truncate px-3 py-2 rounded-lg hover:bg-card transition ${
                  pathname === `/dashboard/c/${c.id}` ? "bg-card" : ""
                }`}
              >
                {c.title}
              </Link>
            ))}
            {chats.length === 0 && <p className="text-xs text-muted px-3 py-2">No chats yet</p>}
          </div>
        </nav>

        <div className="border-t border-border p-3 flex items-center gap-3">
          <div className="size-8 rounded-full bg-grad-primary grid place-items-center text-white text-xs font-semibold">
          ...
          <button onClick={() => signOut({ callbackUrl: "/" })} className="size-8 grid place-items-center rounded-lg hover:bg-card transition text-muted hover:text-text">
